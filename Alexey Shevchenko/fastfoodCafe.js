			...
			this.items.map(function (item) {
				if (item instanceof Drink) {
					...
					console.log('* ' + item.size + ' ' + item.name.toLowerCase() +
						' with ' + item.stuffing.name.toLowerCase() +
						' (' + item.price + '₮ + ' + item.stuffing.calculatePrice() + '₮, ' +
						item.calories + 'cal. + ' + item.stuffing.calculateCalories() + ' cal.)');
				}
				else {
					console.log('* ' + item.name + ' (' + item.calculatePrice() + ' ₮,' + item.calculateCalories() + ' cal.)');
				}
...
/* --- main entry point --- */
var order1 = new fastfoodcafe.Order();
var cola = new fastfoodcafe.Drink(fastfoodcafe.Drink.COLA.name);
order1.getBill();
...
var russian = new fastfoodcafe.Salad(fastfoodcafe.Salad.RUSSIAN.name, 150);
var order2 = new fastfoodcafe.Order(cola, russian);
order2.getBill();
var bigBurger = new fastfoodcafe.Hamburger(fastfoodcafe.Hamburger.SIZE_LARGE.size,
	fastfoodcafe.Hamburger.STUFFING_CHEESE.name);
